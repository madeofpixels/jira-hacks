...
function makeCustomTemplateInjector() {
	...
	// Note: To define a custom template for a new project, add a new entry that follows: '{PROJECT_CODE}': '{web url}';
	const REQUEST_URLS = {
		'CHA': 'https://api.npoint.io/ee12679789551aa03154', // Edit template at https://www.npoint.io/docs/ee12679789551aa03154
		'CHK': 'https://api.npoint.io/eecded9987f7a4a1ee4d', // Edit template at https://www.npoint.io/docs/eecded9987f7a4a1ee4d
		'CHR': 'https://api.npoint.io/beb9027d5be0ebd93da8', // Edit template at https://www.npoint.io/docs/beb9027d5be0ebd93da8
		'CHF': 'https://api.npoint.io/5ffea62497f403179cd7', // Edit template at https://www.npoint.io/docs/5ffea62497f403179cd7
	}
...
function makeFilterBadges() {
	// Map each card color (hex) to it's unique filter
	// The hex key is taken from the [Board settings > Card colors > Color] value (ex: '#35d415')
	// The xpath string is derived from the [Board settings > Card colors > Quick Filters] filter name (ex: PM: To Review)
	const colorFilterMap = {
	  '#35d415': {xpath: '//span[contains(text(),"PM: To Review")]', badge: null, count: 0}
	  , '#eb11d9': {xpath: '//span[contains(text(),"SM: To Review")]', badge: null, count: 0}
	  , '#087dd1': {xpath: '//span[contains(text(),"Team: To Review")]', badge: null, count: 0}
	  , '#f08e16': {xpath: '//span[contains(text(),"EANA: To Review")]', badge: null, count: 0}
	...
	const _getGrabberCounts = function() {
	    ...
	    for (const [colorKey, props] of Object.entries(colorFilterMap)) { props.count = 0; }
	
		// Tally the counts for each filter type
		grabbers.forEach(
		  elt => {
		    const eltColor = _rgbToHex(elt.style.backgroundColor.replace('rgb(','').replace(')','').split(','));
		    if (colorFilterMap[eltColor]) { colorFilterMap[eltColor].count++; }
		  }
		);
	}
	...
	const _addBadgesToFilters = function() {
		...
		for (const [colorKey, props] of Object.entries(colorFilterMap)) {
