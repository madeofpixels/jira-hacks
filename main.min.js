...
return({init:()=>{_init();}});}
function makeFilterBadges(){const BOARD_MUTATION_CONFIG={attributes:false,childList:true,subtree:true};let labelsAndStatusesToFiltersMap={};let boardMutationNode;let isBoardUpdateInProgress=false;const _createBadge=function(count){const badge=document.createElement('span');badge.classList.add('badge');badge.innerHTML=count;return badge;}
...
const _addBadgesToFilters=function(){let currBadge;let filterElt;for(const[label,props]of Object.entries(labelsAndStatusesToFiltersMap)){filterElt=document.evaluate('//span[contains(text(), "'+props.filterName+'")]',document,null,XPathResult.FIRST_ORDERED_NODE_TYPE,null).singleNodeValue;if(filterElt==null){continue;}
...
isBoardUpdateInProgress=true;setTimeout(()=>{const backlogFilterToggle=document.querySelector('#ghx-quick-filters ul:first-child button.css-7uss0q');if(backlogFilterToggle){backlogFilterToggle.click();_getFilterNames();}
