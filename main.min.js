...
function makeCustomTemplateInjector(){const CREATE_ISSUE_MUTATION_CONFIG={attributes:true,childList:true,subtree:true};let currProjectCode;const _fetchProjectTemplates=async function(projectCode,_callback=undefined){const templateSource=(localStorage.getItem('templates-all-projects')!=null)?JSON.parse(localStorage.getItem('templates-all-projects')):USER_TEMPLATE_URLS;const requestURL=templateSource[projectCode];if(localStorage.getItem('template-'+projectCode)!==null||requestURL==undefined){typeof(_callback)=='function'&&_callback(projectCode);return;}
const request=new Request(requestURL);try{const response=await fetch(request);const jsonResponse=await response.json();if(typeof jsonResponse=='object'){localStorage.setItem('template-'+projectCode,JSON.stringify(jsonResponse));}else{throw new Error('_fetchProjectTemplates::Invalid JSON response');}
typeof(_callback)=='function'&&_callback(projectCode);}catch(e){console.error('Error fetching templates: ',e);}}
const _listenForCreateIssueMutations=function(e){setTimeout(()=>{const createIssueMutationNode=document.getElementById('issue-create-modal-dropzone-container');if(createIssueMutationNode!=null){createIssueObserver.observe(createIssueMutationNode,CREATE_ISSUE_MUTATION_CONFIG);}},100);}
const _getIssueTemplate=function(){const projectCodePicker=document.getElementById('issue-create.ui.modal.create-form.project-picker.project-select');const lastCloseEllipseDivIndex=projectCodePicker.innerHTML.lastIndexOf(')</div>');const projectCodeFragment=projectCodePicker.innerHTML.slice(0,lastCloseEllipseDivIndex);const lastOpenEllipseIndex=projectCodeFragment.lastIndexOf('(');const selectedDDProjectCode=(projectCodeFragment&&projectCodeFragment.slice(lastOpenEllipseIndex+1))||null;if(selectedDDProjectCode&&selectedDDProjectCode!=currProjectCode){const createDescriptionField=document.querySelector('.ak-editor-content-area div[aria-label="Main content area"]');createDescriptionField.innerHTML='Loading template...';_fetchProjectTemplates(selectedDDProjectCode,_injectIssueTemplate);}else{_injectIssueTemplate(currProjectCode);}}
...
return;}}}
const createIssueObserver=new MutationObserver(_onCreateIssueMutation);const _clearTemplatesIfExpired=function(){if(localStorage.getItem('templates-created-on-date')!==null){const templatesCreatedOnDate=new Date(localStorage.getItem('templates-created-on-date'));const currDate=new Date();const diffDatesByTime=currDate.getTime()-templatesCreatedOnDate.getTime();const diffDatesByDays=diffDatesByTime/(1000*3600*24);if(diffDatesByDays>7){for(key in localStorage){if(key.search(/^(template)(s?)(-)/)==0){localStorage.removeItem(key);}}
localStorage.setItem('templates-created-on-date',new Date());}}else{localStorage.setItem('templates-created-on-date',new Date());}}
const _getProjectTemplateUrls=async function(_callback=undefined){_clearTemplatesIfExpired();if(localStorage.getItem('templates-all-projects')!=null||typeof(USER_TEMPLATE_URLS)!='undefined'){typeof(_callback)=='function'&&_callback();}else if(typeof(REMOTE_PROJECT_TEMPLATES_URL)!='undefined'&&localStorage.getItem('templates-all-projects')==null){try{const request=new Request(REMOTE_PROJECT_TEMPLATES_URL);const response=await fetch(request);const jsonResponse=await response.json();if(typeof jsonResponse=='object'){localStorage.setItem('templates-all-projects',JSON.stringify(jsonResponse));}else{throw new Error('getProjectTemplateUrls::Invalid JSON response');}
typeof(_callback)=='function'&&_callback();}catch(e){console.error('Error fetching templates: ',e);}}}
const _onUpdate=function(){let newProjectPathname=window.location.pathname.split('projects/');let newProjectCode=newProjectPathname.length==1?undefined:newProjectPathname[1].split('/')[0];setTimeout(()=>{document.getElementById('createGlobalItem').addEventListener('click',_listenForCreateIssueMutations);document.getElementById('createGlobalItemIconButton').addEventListener('click',_listenForCreateIssueMutations);},10);if(newProjectCode==undefined){return;}
_getProjectTemplateUrls(()=>{if(newProjectCode!=currProjectCode){currProjectCode=newProjectCode;_fetchProjectTemplates(currProjectCode);}});}
